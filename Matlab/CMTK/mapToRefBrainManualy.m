function mapToRefBrainManualy(RefBrain, RefBrainPath, RegistrationPath, mode, transformation, movPath, mov)
...
% mov   moving image if necessary
    ...
    refPath = fullfile(RefBrainPath, RefBrain); % points to the refbrain in the refbrain folder
    regPath = fullfile(RegistrationPath,refBrainName); % points to the refbrain in the registration folder
    movPath = [movPath]; % ex: if mov is 'graystack' tag, movPath will point to 'graystack.nhdr'
