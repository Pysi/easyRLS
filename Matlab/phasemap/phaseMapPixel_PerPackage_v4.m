...
N = TimewindowDFF(2) - TimewindowDFF(1);                      % Number of images per layer
...
for iz = Zlay
    ...
    for k = 1 : packageSize
        ...
        DFF_fft = fft(mdff.Data.bit(TimewindowDFF(1):TimewindowDFF(2), i),[],1);
        phase_DFF_fft_tw = angle(DFF_fft(fw_index,:)) ;
        
        %deltaphi_tw = phase_DFF_fft_tw - phase_Stim_fw ;
        deltaphi_tw = phase_Stim_fw - phase_DFF_fft_tw;
        ...
        phase     = angle(DFF_fft(ind_fstim,:));
        %  realpart  = real(Y(ind_fstim,:));
        %  imaginary = imag(Y(ind_fstim,:));
        %deltaphi  = (phase - phase_delay + pi);
        
        %deltaphi  = deltaphi_mean;
        deltaphi  = deltaphi_mean - phase_delay;
